...
class BoughtTripsListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val toastLoad = Toast.makeText(requireContext(), "Loading trips...", Toast.LENGTH_SHORT)
        toastLoad.show()//show the toast
        val db = FirebaseFirestore.getInstance()//create firestore instance
        //get all the trips created by the user
        val rv = view.findViewById<RecyclerView>(R.id.recyclerViewBought)
            ...
            .addOnSuccessListener() { documents ->
                if (documents?.isEmpty == false) {
                    for (thisTrip in documents) {
                        ...
                        rv.adapter = TripAdapterBought(data, findNavController())
                    }
                } else {
                    toastLoad.cancel()
...
class TripAdapterBought(val data: List<TripOthers>, val navController: NavController) : RecyclerView.Adapter<TripAdapterBought.TripViewHolder>() {
        ...
        fun bind(t: TripOthers, navController: NavController) {
            ...
            cardView.setOnClickListener {
                navController.navigate(
                    R.id.action_boughtTripsListFragment_to_nav_trip_detail3, bundleOf(
                        "TripID" to t.id,
